...
export default function Page() {
  ...
  // NEW: categories is now state, seeded from step 1 or defaults
    ...
    <div>
      <div className="min-h-screen bg-white pr-[20%] pl-[20%] pt-[8%] pb-[5%]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full">
          {categories.map((cat) => (
            <div key={cat} className={`p-4 rounded-3xl ${css(status[cat])}`}>
              <label className="block text-black capitalize pl-2 pt-2">
                {cat}
              </label>
              <textarea
                ref={(el) => {
